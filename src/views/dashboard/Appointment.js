import { Button, Tooltip, theme, Table, Space } from "antd";
...
function Seekers() {
  ...
  async function deleteItem() {
    const apiResponse = await DeleteAppointmentApi(selectItem.id);
    if (
  ...
  const columns = [
    {
      ...
      dataIndex: "name",
      key: "name",
      render: (text, record) => <h1>{record.seeker.name}</h1>,
    },
    {
      title: "Address",
      dataIndex: "address",
      key: "address",
      render: (text, record) => <h1>{record.seeker.address}</h1>,
    },
    {
      ...
      render: (text, record) => (
        <h1>
          {record.consultant.user.name} ({record.consultant.country})
        </h1>
      ),
    },

    {
      ...
      dataIndex: "date",
      key: "date",
      render: (text, record) => <h1>{GetDate(record.date)}</h1>,
    },

    {
      ...
      dataIndex: "startTime",
      key: "startTime",
      render: (text, record) => <h1>{GetTime(record.startTime)}</h1>,
    },
    {
      title: "End Time",
      dataIndex: "endTime",
      key: "endTime",
      render: (text, record) => <h1>{GetTime(record.endTime)}</h1>,
    },
    {
      title: "Seeker Job Type",
      dataIndex: "job_type",
      key: "job_type",
      render: (text, record) => <h1>{record.seeker.job_type}</h1>,
    },

    {
      ...
      render: (text, record) => (
        <Space size="middle">
          <Tooltip title={"Edit"}>
            <div
              className="text-blue-600  text-2xl hover:text-blue-600 cursor-pointer"
              onClick={() => {
          ...
          <Tooltip title={"Delete"}>
            <div
              ...
              className="text-red-600 text-2xl hover:text-red-600 cursor-pointer"
            >
              <DeleteOutlined />
            </div>
          </Tooltip>
    ...
    <>
      <>
        <Tooltip placement="left" title={"Add categories"}>
          <Button
                style={{ background: token.colorPrimary,width:"160px" }}
                shape="default"
                type="primary"
            ...
            >Add Appointment <PlusOutlined /></Button>
        </Tooltip>
      </>
    </>
  );

  const left = <div className="pri-font text-xl font-semibol">Appointments</div>;
  return (
    <>
