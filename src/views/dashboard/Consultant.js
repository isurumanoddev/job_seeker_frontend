import { Button, Table, Tooltip, theme, Space } from "antd";
...
function Consultant() {
  ...
  async function deleteConsultant() {
    const apiResponse = await DeleteConsultantApi(selectItem.id);
    if (
  ...
  const columns = [
    {
      ...
      dataIndex: "name",
      key: "name",
      render: (text, record) => <h1>{record.user.name}</h1>,
    },
        ...
        <>
          <Tooltip placement="left" title={"Add Consultant"}>
            <Button
              ...
              onClick={() => setAddModal(true)}
            >Add Consultant<PlusOutlined /></Button>
          </Tooltip>
        </>
      )}
    </>
  ...
  const left = <div className="pri-font text-xl font-semibol">Consultants</div>;
